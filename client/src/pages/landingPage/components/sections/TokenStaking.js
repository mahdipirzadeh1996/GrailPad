...
import grailImg from '../../assets/images/LOGO-Final-e1636643001227.png'
...
function TokenStaking({ handleTokensChange }) {
    ...
    useEffect(() => {
        ...
        setGrailTokenContract(getGrailTokenContractInstance(web3))
    }, [web3]);
    ...
    const handleSubmitStake = async () => {
        ...
        try {
            await grailTokenContract.methods.approve(tokenFarmContract._address, stakedAmountInWei).send({ from: account, gas: '2000000' }).on('receipt', async (txReceiptApproved) => {
    ...
    const renderWithdrawButtonOrCountdown = () => {
        if (timeOfStaking) {
            ...
            console.log(moment.unix(parseInt(timeOfStaking)).format('MM DD YYYY, h:mm a'))
            return (
                diff > 0 ?
                    <Countdown
                ...
                <div className='input-container'>
                    <Form className='input-container__form'>
                        ...
                        >
                            ...
                            <Label basic image className='dai-label-and-image'><Image src={grailImg} className='input-field__image' /><span className='dai-label'>GRAIL</span></Label>
                        </Form.Input>
                        <Button disabled={isDisabled} className='input-container__button' onClick={handleSubmitStake}>Stake GRAIL tokens</Button>
