...
import { checkDaiTokensFor, checkGrailTokensFor, checkMetoTokensFor, convertToTokens } from '../../utils/assets'
// Layouts
...
const LandingPage = () => {
    ...
    const handleTokensChange = useCallback(async () => {
        ...
        const grailToken = await checkGrailTokensFor(web3, account);
        setGrailTokens(grailToken);
        //console.log(grailToken / 10 ** 18);
    ...
    return (
        <>
            <ScrollReveal
                ref={childRef}
                children={() => (
                    <Layout metoTokens={metoTokens} daiTokens={daiTokens} grailTokens={grailTokens}>
